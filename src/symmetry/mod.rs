...
/// Supported historical name for [`canonical`].
///
/// ```
/// use venn_search::geometry::NCOLORS;
/// use venn_search::symmetry::{canonical, s6};
///
/// let degrees = [3; NCOLORS];
/// let result: venn_search::SymmetryType = s6::check_symmetry(&degrees);
/// assert_eq!(result, canonical::SymmetryType::Equivocal);
/// assert_eq!(result, venn_search::check_symmetry(&degrees));
/// let _: fn(&venn_search::context::DynamicState, &venn_search::context::MemoizedData)
///     -> s6::SymmetryType = s6::check_solution_canonicality;
/// ```
pub use canonical as s6;
